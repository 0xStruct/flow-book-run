...
export const codeExecutor: (context: vscode.ExtensionContext) => Executor = (context) => (
	...
	return new Promise(async (c, e) => {
		...
		try {
			...
			const commands = vscode.workspace.getConfiguration('flow-notebook').get('dispatch') as Record<string, [string, string[], Record<string, string>]>;
			if (!commands[language] || !commands[language][0] || !commands[language][1]) {
				logger(`Your Flow Notebook cannot execute ${language || `_undefined lang_`} cells.`);
				return c(undefined);
			}
			...
			// write temp file
    		fs.writeFileSync(cwd +'/test-fcl.js', text);
...
export const makeNotebookController = (controllerId: string, notebookId: string, label: string, executor: Executor): vscode.Disposable => {
	...
	controller.supportedLanguages = Object.keys(vscode.workspace.getConfiguration('flow-notebook').get('dispatch') ?? {});

	const languageSupporter = vscode.workspace.onDidChangeConfiguration(e => {
		if (e.affectsConfiguration('flow-notebook.dispatch')) {
			controller.supportedLanguages = Object.keys(vscode.workspace.getConfiguration('flow-notebook').get('dispatch') ?? {});
		}
	});
